 ...
 * 
 */
@JobHandler(value="auditJobHandler")
@Component
public class AuditJobHandler extends IJobHandler {
	...
	public ReturnT<String> execute(String arg0) throws Exception {
		XxlJobLogger.log("xxl-job----kcms audit job start");
		//根据当前时间获取当前季度
		...
		if(nowQuarter.equals(lastQuarter)) {
			flagA = softwareListService.audit(nowQuarter);
			XxlJobLogger.log(String.valueOf(flagA));
		}else {
			//不同则同时汇算两个季度
			flagB = softwareListService.audit(lastQuarter);
			XxlJobLogger.log(String.valueOf(flagB));
			
			flagC = softwareListService.audit(nowQuarter);
			XxlJobLogger.log(String.valueOf(flagC));
		}
		if(flagA || (flagB && flagC)) {
			XxlJobLogger.log("xxl-job----kcms audit job end success");
			return SUCCESS;
		}else {
			XxlJobLogger.log("xxl-job----kcms audit job end fail");
			return FAIL;
		}
	}
