 ...
 * 
 */
@JobHandler(value="weihuJobHandler")
@Component
public class WeihuJobHandler extends IJobHandler {
	...
	public ReturnT<String> execute(String arg0) throws Exception {
		XxlJobLogger.log("xxl-job----kcms weihu job start");
		//根据当前时间获取当前季度
		...
		if(nowQuarter.equals(lastQuarter)) {
			flagA = softwareListService.weihu(nowQuarter);
			XxlJobLogger.log(String.valueOf(flagA));
		}else {
			//不同则同时汇算两个季度
			flagB = softwareListService.weihu(lastQuarter);
			XxlJobLogger.log(String.valueOf(flagB));
			
			flagC = softwareListService.weihu(nowQuarter);
			XxlJobLogger.log(String.valueOf(flagC));
		}
		if(flagA || (flagB && flagC)) {
			XxlJobLogger.log("xxl-job----kcms weihu job end success");
			return SUCCESS;
		}else {
			XxlJobLogger.log("xxl-job----kcms weihu job end fail");
			return FAIL;
		}
	}
