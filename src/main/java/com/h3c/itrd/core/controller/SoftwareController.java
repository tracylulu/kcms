...
//@ApiIgnore
@Api(value = "软件开发计算得分模块相关接口", tags = {"软件开发计算得分模块相关接口"})
public class SoftwareController {
	private static Logger logger = LoggerFactory.getLogger(SoftwareController.class);
    ...
    @GetMapping("/devSu")
    @ApiOperation(value = "开发得分计算，后续改成定时任务触发—dev_su")
    public ResponseResult devSu() throws Exception {
        ...
        for (int i = 0; i <splitList1.size() ; i++) {
            ...
            for (int j = 0; j < limitList.size(); j++) {
            	//if(limitList.get(j).getProjectId().equals("43BF098E-DA69-478D-98AC-874B1F169D08")) {
	...
	private Map<String,Double> getWorktime(String start, String end, String quarter, String pauseStart1,
        ...
        //项目总时长=有效工作日-暂停工作日(未结束的项目不需要去除暂停时间)
  		...
  		if (days <= 0){
			...
			//未结算的项目查询当季度交集了多少个工作日（不包括定时算分的今天），结束的项目不减1------20210610讨论后决定不减1
			//if(isOver == 1) {
...
//			}
    ...
    @GetMapping("/develop")
    @ApiOperation(value = "云数软件得分计算，后续改成定时任务触发—develop")
    public ResponseResult develop() throws Exception{
        String quarter = DateTimeUtils.getQuarter();
        //String quarter="2021Q1";
    ...
    @GetMapping("/audit")
    @ApiOperation(value = "评审得分计算，后续改成定时任务触发—audit")
    public ResponseResult audit() throws Exception{
        String quarter = DateTimeUtils.getQuarter();
        //String quarter ="2021Q1";
    ...
    @GetMapping("/platform")
    @ApiOperation(value = "平台贡献得分计算，后续改成定时任务触发—platform")
    public ResponseResult platform() throws Exception{
        String quarter = DateTimeUtils.getQuarter();
        //String quarter ="2021Q1";
        //删除历史数据的代码置后。//删除历史数据的代码置后。（不置后，用事务来实现代码报错删除操作回滚）
    	Map<String, Object> deleteParam = new HashMap<>();
    ...
    @GetMapping("/weihu")
    @ApiOperation(value = "维护得分计算，后续改成定时任务触发—weihu")
    public ResponseResult weihu() throws Exception{
        String quarter = DateTimeUtils.getQuarter();
    	//String quarter ="2021Q1";
        //删除历史数据的代码置后。（不置后，用事务来实现代码报错删除操作回滚）
    ...
    @ApiOperation(value = "当季得分计算，后续改成定时任务触发—total_score")
    public ResponseResult totalScore() throws Exception{
        String quarter = DateTimeUtils.getQuarter();
        //String quarter ="2021Q1";
        //删除历史数据的代码置后。（不置后，用事务来实现代码报错删除操作回滚）
...
//        }
    ...
    @GetMapping("/devModelV")
    @ApiOperation(value = "开发得分计算(isoftV模型项目)，后续改成定时任务触发—dev_modelV")
    public ResponseResult devModelV() throws Exception {
    	String quarter = DateTimeUtils.getQuarter();
    	//String quarter ="2021Q1";
    	String a1 = quarter.substring(0, quarter.indexOf("Q"));
    	String a2 = quarter.substring(a1.length(), quarter.length());
        ...
        for (int i = 0; i <splitList1.size() ; i++) {
            ...
            for (int j = 0; j < limitList.size(); j++) {
            	String projectId=limitList.get(j).getProjectId();
            	//if(projectId.equals("A4949AE9-0426-4F1E-BE08-A086187B07BE")) {
            	//if(projectId.equals("018DAC7F-92D3-11EB-862C-FA163E762A0D")) {
            	if(projectId.equals("B45EE8DB-5D2E-4385-8017-BA779AFBA523")) {
            	//流水号信息
    ...
    private Map<String, List> devForModelV(IsoftProjectListDO v, List<IsoftContributionListDO> kx,
...
//        double W1XFull=0;
//        for (int i = 0; i < quarterWork.size(); i++) {
//        	W1XFull+=quarterWork.get(i).getProQuarWork().doubleValue();
//    		if(quarterWork.get(i).getQuarterStr().equals(DateTimeUtils.getQuarter1())) {
//				W1X=quarterWork.get(i).getProQuarWork().doubleValue();
//			}
//		}
//        //项目当季总得分（满分）SA = （Q1X*W1X*参数【项目工作量系数】）/100，Q1X默认为100
//        double saFull=(W1X*config.getVProWork().doubleValue())/100;
//        //如果一个项目跨q2,q3,q4季度。则q2按照一个方法算，q3,q4按照另一种算，用项目开始时间算。和之前的软件的不一样，之前的是q2,q3一起算，q4单算
//        //本季度项目或者跨季度第一季度。当季项目实际得分SA = （Q1X*W1X*参数【项目工作量系数】）/100，Q1X为实际值；
//        double sa=0;
//        //个人项目得分
//        double saPerson=0;
//        //之前季度归档季度的实际项目得分
...
//        	//个人项目得分（计算项目每一个成员的得分）
//        	// Kx为分摊比例。按照贡献度表中的对应项目的人员贡献度算每个人的分数
//        	int proQuestDI=0;//项目关联问题单总DI
//        	for (int i = 0; i < idmsList.size(); i++) {
//        		proQuestDI += (int)SystemCommonUtil.getIdmsType(idmsList.get(i).getOdcSeverity());
//			}
//            for (int j = 0; j < kx.size(); j++) {
...
//            	for (int k = 0; k < idmsList.size(); k++) {
//            		if(idmsList.get(k).getSubmitBy().equalsIgnoreCase(kx.get(j).getPerson())){
//            			subQuestDI+= (int)SystemCommonUtil.getIdmsType(idmsList.get(k).getOdcSeverity());
//            		}
//            		if(idmsList.get(k).getNameB().equalsIgnoreCase(kx.get(j).getPerson())){
//            			solveQuestDI+= (int)SystemCommonUtil.getIdmsType(idmsList.get(k).getOdcSeverity());
//            			for (int m = 0; m < problemsList.size(); m++) {
//    						if(idmsList.get(k).getDefectNO().equals(problemsList.get(m).getDefectNo())) {
//    							checkSolveQDI+= (int)SystemCommonUtil.getIdmsType(idmsList.get(k).getOdcSeverity());
//    						}
//            			}
//					}
//				}
...
//            	projectProblemsDiDO.setProjectName(v.getProjectName());
//            	projectProblemsDiDO.setAccount(kx.get(j).getPerson());
//            	projectProblemsDiDO.setSubQuestDI(subQuestDI);
//            	projectProblemsDiDO.setSolveQuestDI(solveQuestDI);
//            	projectProblemsDiDO.setCheckSolveQDI(checkSolveQDI);
//            	projectProblemsDiDO.setProQuestDI(proQuestDI);
//            	projectProblemsDiDO.setCountDate(new Date());
//            	projectProblemsDiList.add(projectProblemsDiDO);
//            	
//            	//本季度、跨季度第一季度个人项目得分:
//            	//个人项目得分 = (项目当季工作量*参数【项目工作量系数】- 系统测试总DI*系数A - 验收测试解单总DI*系数B)*质量得分*项目当季个人贡献度  + 
//            	//[(个人提单的系统测试总DI值*提单系数 + 个人解单（系统测试单）总DI值*解单系数)*系数A +个人验收测试解单总DI*系数B]*质量得分
////            	saPerson=W1X*config.getVProWork().doubleValue()-proQuestDI*config.getVSystestdiA().doubleValue()-
////            	
////            	先把该项目跨那些季度总结下？循环每个季度？
////            	for(i){
//            	}
////            	
////            	
////            	//project_summary_V  v模型的汇算中间表中赋值
////            	ProjectSummaryVDO projectSummaryVDO=new ProjectSummaryVDO();
////            	projectSummaryVDO.setProjectId(projectId);
////            	projectSummaryVDO.setProjectName();
////            	projectSummaryVDO.setQuarter(quarter);
////            	projectSummaryVDO.setAccount(account);
////            	projectSummaryVDO.set
////            	projectSummaryVDO.set
////            	projectSummaryVDO.set
////            	projectSummaryVDO.set
////            	projectSummaryVDO.set
...
//            	
//            	
//            	//往project_summary表中赋值
//            	//计算每一个成员的扣除得分（相比于之前的软件算法用的是S1X，现在用当即工作量代替）
//            	double Amin = (1-A/100)*config.getPrjAtprefix().doubleValue() * W1X;
//            	double Bmin = (1-B/100)*config.getPrjScheduleprefix().doubleValue() *W1X;
//            	//当鉴定得分不计算时，为NA
//            	double Cmin = noCodeAuth ? -1:(1-C/100)*config.getPrjCodeauthprefix().doubleValue() *W1X;  
...
//        	
////        }else {
////        	//跨季度其他季度。跨季度项目：（每个季度工作量之和*参数【项目工作量系数】*Q1X）/100-之前季度归档季度的实际项目得分
////        	//当前季度归档日（后推30天）。如果归档日还没计算出来，则取前一天的最新得分。比如4.15号
////        	//有问题，要修改
////        	long gdTime=endDate1.getTime()+30*3600*24;
////        	SimpleDateFormat format =  new SimpleDateFormat("yyyy-MM-dd");
////        	String d = format.format(gdTime);
////        	boolean haveGD=prov.stream().anyMatch(o -> o.getCountDate().equals(d));
////        	if(haveGD) {
////        		saGD=prov.stream().filter(o->o.getCountDate().equals(d)).collect(Collectors.toList()).get(0).getProScored().doubleValue();
////        	}else {
////        		//saGD=prov.stream().filter(o->o.getCountDate().equals(DateTimeUtils.getlastDate())).collect(Collectors.toList()).get(0).getProScored().doubleValue();
////        	}
////        	sa=(Q1X*W1XFull*config.getVProWork().doubleValue())/100-saGD;
////        	
////        	//个人项目得分（计算项目每一个成员的得分）
////        	
////        	
...
//        
    ...
    @ApiOperation(value = "测试批量增加，notice表举例--测试")
    public ResponseResult testSaveBatch() {
        List<NoticeListDO> noticeListDOList=new ArrayList<>();
        for (int i = 0; i <100 ; i++) {
            NoticeListDO noticeListDO=new NoticeListDO();
            noticeListDO.setTime("123");
            noticeListDO.setNotice(""+i);
            noticeListDOList.add(noticeListDO);
        }
        boolean b = noticeListService.saveBatch(noticeListDOList);
